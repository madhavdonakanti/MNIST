...
for u in range(epochs):
    #training over the batches
    batch_size=256
    numbatches=math.ceil(m/batch_size)
    ...
    for i in range(numbatches):
        ...
        #converting to matrices
        inp=indf.iloc[i*batch_size:(i+1)*batch_size,:].to_numpy()
        out=outdf.iloc[i*batch_size:(i+1)*batch_size,:].to_numpy()

        sampsize=inp.shape[0] #number of samples in the batch

        Y=np.zeros((sampsize,10))
        ...
        #calculating cost
        cost=costcalc(Y,listafac,sampsize)
        ...
        #Backpropogation
        #for output layer
        dc_dzL=(1/sampsize)*(listafac[L-1]-Y)
        assert dc_dzL.shape==(n[L],sampsize)
        ...
        for k in range(L-1):
            ...
            assert dc_dzl.shape==(n[L-1-k],sampsize)
            if L-3-k >= 0:
    ...
    if u%50==0:
        print(costs[(u+1)*numbatches-1])  #printing the cost for the last batch of the epoch
        print(costsv[u])

fig, axes = plt.subplots(2, 1)
axes[0].plot(range(len(costs)),costs)
axes[1].plot(range(len(costsv)),costsv)
plt.show()
